...
def merge_odds_lists(list1: List[dict], list2: List[dict]) -> str:
            ...
            if game1['time'] == game2['time']:
                ...
                if set(team1_split1[:1]).intersection(team2_split1[:1]) and set(team1_split2[:1]).intersection(team2_split2[:1]):
                    merged_list.append({
                        ...
                        'arbitrage': caller_function(game1['home_odds'],game1['away_odds'],game1['draw_odds'],game2['home_odds'], game2['away_odds'],game2['draw_odds'])
                    })
    ...
    table = PrettyTable()
    table.field_names = ["Date", "SportPesa Home Team", "SportPesa Away Team", "SportPesa Home Odds", "SportPesa Away Odds", "SportPesa Draw Odds", "Betika Home Team", "Betika Away Team", "Betika Home Odds", "Betika Away Odds", "Betika Draw Odds", "Arbitrage"]
    for row in merged_list:
        table.add_row([row['time'], row['home_team1'], row['away_team1'], row['home_odds1'], row['away_odds1'], game1['draw_odds'], row['home_team2'], row['away_team2'], row['home_odds2'], row['away_odds2'], game2['draw_odds'], row['arbitrage'] ])
    return str(table)
...
def arbitrage_calc(odds1, odds2, odds3, total_stake):
    ...
    fair_odds3 = prob3 / total_prob

    # Calculate the stake for each outcome
    stake1 = (total_stake * fair_odds1) / (fair_odds1 + fair_odds2 + fair_odds3)
    stake2 = (total_stake * fair_odds2) / (fair_odds1 + fair_odds2 + fair_odds3)
    stake3 = (total_stake * fair_odds3) / (fair_odds1 + fair_odds2 + fair_odds3)

    # Calculate the possible winnings for each outcome
    winnings1 = stake1 * odds1
    winnings2 = stake2 * odds2
    winnings3 = stake3 * odds3
    ...
    return {f'stake1 for Bookmaker 1 with odds {odds1}': round(stake1, 2),
            ...
            f'stake3 for Bookmaker 3 with odds {odds3}': round(stake3, 2),
            'winnings1': round(winnings1, 2),
            'winnings2': round(winnings2, 2),
            'winnings3': round(winnings3, 2),
            'guaranteed_profit': round(guaranteed_profit, 2)}


def caller_function(odds_1_player_1, odds_1_player_2, odds_1_draw, odds_2_player_1, odds_2_player_2, odds_2_draw):
    odds_1_player_1 = float(odds_1_player_1)
    odds_1_player_2 = float(odds_1_player_2)
    odds_1_draw = float(odds_1_draw)
    odds_2_player_1 = float(odds_2_player_1)
    odds_2_player_2 = float(odds_2_player_2)
    odds_2_draw = float(odds_2_draw)

    # Arbitrage calculations
    prob_player_1_bookmaker_1 = 1 / odds_1_player_1
    prob_player_1_bookmaker_2 = 1 / odds_2_player_2
    prob_player_1_draw = 1 / odds_1_draw
    total_prob_player_1 = prob_player_1_bookmaker_1 + prob_player_1_bookmaker_2 + prob_player_1_draw

    print("Prob 1", total_prob_player_1)
    prob_player_2_bookmaker_1 = 1 / odds_1_player_2
    prob_player_2_bookmaker_2 = 1 / odds_2_player_1
    prob_player_2_draw = 1 / odds_2_draw
    total_prob_player_2 = prob_player_2_bookmaker_1 + prob_player_2_bookmaker_2 + prob_player_2_draw
    ...
    if arbitrage_margin_player_2 > 0:
        ...
        print(f"Bookmaker 1 with odds {odds_1_player_2}, {odds_1_draw} || and Bookmaker 2 with odds {odds_2_player_1}, {odds_2_draw}")
        odds_list = [odds_1_player_2, odds_1_draw, odds_2_player_1]
        potential_profit = arbitrage_calc(odds_1_player_2, odds_1_draw, odds_2_player_1, 2000)
        return odds_list, potential_profit
    elif arbitrage_margin_player_1 > 0:
        print("Arbitrage opportunity found!")
        print(f"Bookmaker 1 with odds {odds_1_player_1}, {odds_1_draw} || and Bookmaker 2 with odds {odds_2_player_2}, {odds_2_draw}")
        odds_list = [odds_1_player_1, odds_1_draw, odds_2_player_2]
        potential_profit = arbitrage_calc(odds_1_player_2, odds_1_draw, odds_2_player_1, 2000)
        return odds_list, potential_profit
    else:
